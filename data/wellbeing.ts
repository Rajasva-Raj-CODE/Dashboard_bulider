...
export const wellbeingHeatmap = { labels:{ x:['Mon','Tue','Wed','Thu','Fri'], y:['W1','W2','W3','W4'] }, datasets:[{ label:'Participation', data:Array.from({length:20},(_,i)=>({x:i%5,y:Math.floor(i/5),v:Math.floor(Math.random()*10)})), backgroundColor(ctx:{ raw: { v: number } }){ const v=ctx.raw.v; return v>6?'#F472B6':v>3?'#FBCFE8':'#FCE7F3'; }, width:({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height:({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const wellbeingTreemap = { datasets:[{ tree:[ { value:64, label:'Yoga' }, { value:35, label:'Counseling' }, { value:88, label:'Steps' }, { value:42, label:'Mindfulness' }, { value:51, label:'Nutrition' } ], key:'value', groups:['label'], backgroundColor:'#FBCFE8', spacing:0.5, labels:{ display:true, formatter:(ctx:{ raw: { g: string } })=>ctx.raw.g } }] };
