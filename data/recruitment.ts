...
export const recruitmentHeatmap = { labels: { x: ['Mon','Tue','Wed','Thu','Fri'], y: ['W1','W2','W3','W4'] }, datasets: [{ label:'Applications', data: Array.from({ length:20 }, (_,i)=>({ x:i%5, y:Math.floor(i/5), v: Math.floor(Math.random()*20)})), backgroundColor(ctx:{ raw: { v: number } }){ const v=ctx.raw.v; return v>12?'#60A5FA':v>6?'#93C5FD':'#DBEAFE'; }, width: ({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height: ({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const recruitmentTreemap = { datasets: [{ tree: [ { value: 9, label: 'Backend' }, { value: 7, label: 'Frontend' }, { value: 5, label: 'QA' }, { value: 3, label: 'PM' } ], key:'value', groups:['label'], backgroundColor:'#DDD6FE', spacing:0.5, labels:{ display:true, formatter:(ctx:{ raw: { g: string } })=>ctx.raw.g } }] };
