...
export const helpdeskHeatmap = { labels: { x: ['Mon','Tue','Wed','Thu','Fri'], y: ['W1','W2','W3','W4'] }, datasets: [{ label: 'Tickets', data: Array.from({ length:20 }, (_,i)=>({ x:i%5, y:Math.floor(i/5), v: Math.floor(Math.random()*12)})), backgroundColor(ctx:any){ const v=ctx.raw.v; return v>8?'#F59E0B':v>4?'#FBBF24':'#FEF3C7'; }, width: ({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height: ({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const helpdeskTreemap = { datasets: [{ tree: [ { value: 42, label: 'Software' }, { value: 38, label: 'Hardware' }, { value: 12, label: 'Network' }, { value: 22, label: 'Access' } ], key:'value', groups:['label'], backgroundColor:'#FDE68A', spacing:0.5, labels:{ display:true, formatter:(ctx:any)=>ctx.raw.g } }] };
