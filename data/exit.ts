import { DoorOpen, MessageCircleWarning, ClipboardList, Users, Percent, CalendarDays, UserCheck2, Star, StarHalf } from 'lucide-react';
...
export const exitHeatmap = { labels:{ x:['Mon','Tue','Wed','Thu','Fri'], y:['W1','W2','W3','W4'] }, datasets:[{ label:'Exit Events', data:Array.from({length:20},(_,i)=>({x:i%5,y:Math.floor(i/5),v:Math.floor(Math.random()*4)})), backgroundColor(ctx:any){ const v=ctx.raw.v; return v>2?'#F87171':v>1?'#FCA5A5':'#FEE2E2'; }, width:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const exitTreemap = { datasets:[{ tree:[ { value:5, label:'Eng' }, { value:3, label:'Sales' }, { value:2, label:'HR' }, { value:1, label:'Ops' } ], key:'value', groups:['label'], backgroundColor:'#FCA5A5', spacing:0.5, labels:{ display:true, formatter:(ctx:any)=>ctx.raw.g } }] };
