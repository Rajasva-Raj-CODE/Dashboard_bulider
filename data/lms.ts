...
export const lmsHeatmap = { labels:{ x:['Mon','Tue','Wed','Thu','Fri'], y:['W1','W2','W3','W4'] }, datasets:[{ label:'Sessions', data:Array.from({length:20},(_,i)=>({x:i%5,y:Math.floor(i/5),v:Math.floor(Math.random()*10)})), backgroundColor(ctx:any){ const v=ctx.raw.v; return v>6?'#0EA5E9':v>3?'#7DD3FC':'#E0F2FE'; }, width:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const lmsTreemap = { datasets:[{ tree:[ { value:34, label:'Tech' }, { value:26, label:'Soft Skills' }, { value:18, label:'Leadership' }, { value:12, label:'Compliance' } ], key:'value', groups:['label'], backgroundColor:'#BAE6FD', spacing:0.5, labels:{ display:true, formatter:(ctx:any)=>ctx.raw.g } }] };
