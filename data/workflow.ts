...
export const workflowHeatmap = { labels:{ x:['Mon','Tue','Wed','Thu','Fri'], y:['W1','W2','W3','W4'] }, datasets:[{ label:'Throughput', data:Array.from({length:20},(_,i)=>({x:i%5,y:Math.floor(i/5),v:Math.floor(Math.random()*10)})), backgroundColor(ctx:any){ const v=ctx.raw.v; return v>6?'#3B82F6':v>3?'#93C5FD':'#DBEAFE'; }, width:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const workflowTreemap = { datasets:[{ tree:[ { value:18, label:'Onboarding' }, { value:12, label:'Expense' }, { value:10, label:'Leave' }, { value:8, label:'Access' } ], key:'value', groups:['label'], backgroundColor:'#BFDBFE', spacing:0.5, labels:{ display:true, formatter:(ctx:any)=>ctx.raw.g } }] };
