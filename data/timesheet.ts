...
export const timesheetHeatmap = { labels:{ x:['Mon','Tue','Wed','Thu','Fri'], y:['W1','W2','W3','W4'] }, datasets:[{ label:'Hours', data:Array.from({length:20},(_,i)=>({x:i%5,y:Math.floor(i/5),v:Math.floor(Math.random()*10)})), backgroundColor(ctx:any){ const v=ctx.raw.v; return v>6?'#2563EB':v>3?'#93C5FD':'#DBEAFE'; }, width:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height:({chart}:any)=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const timesheetTreemap = { datasets:[{ tree:[ { value:2100, label:'Alpha' }, { value:1900, label:'Beta' }, { value:1720, label:'Gamma' }, { value:1500, label:'Delta' } ], key:'value', groups:['label'], backgroundColor:'#C7D2FE', spacing:0.5, labels:{ display:true, formatter:(ctx:any)=>ctx.raw.g } }] };
