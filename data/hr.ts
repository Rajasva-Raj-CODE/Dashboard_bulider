...
export const hrHeatmap = { labels: { x: ['Mon','Tue','Wed','Thu','Fri'], y: ['W1','W2','W3','W4'] }, datasets: [{ label: 'Interviews', data: Array.from({ length:20 }, (_,i)=>({ x: i%5, y: Math.floor(i/5), v: Math.floor(Math.random()*10)})), backgroundColor(ctx:{ raw: { v: number } }){ const v=ctx.raw.v; return v>6?'#6366F1':v>3?'#A5B4FC':'#E0E7FF'; }, width: ({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{ const ca=chart?.chartArea; return ca? (ca.width/5)-6:20; }, height: ({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{ const ca=chart?.chartArea; return ca? (ca.height/4)-6:20; } }] };
export const hrTreemap = { datasets: [{ tree: [ { value: 120, label: 'Engineering' }, { value: 80, label: 'Sales' }, { value: 40, label: 'HR' }, { value: 35, label: 'Finance' } ], key: 'value', groups:['label'], backgroundColor: '#93C5FD', spacing: 0.5, labels:{ display:true, formatter:(ctx:{ raw: { g: string } })=>ctx.raw.g } }] };
