...
export const pmsHeatmap = { labels:{ x:['Mon','Tue','Wed','Thu','Fri'], y:['W1','W2','W3','W4'] }, datasets:[{ label:'Check-ins', data:Array.from({length:20},(_,i)=>({x:i%5,y:Math.floor(i/5),v:Math.floor(Math.random()*10)})), backgroundColor(ctx:{ raw: { v: number } }){ const v=ctx.raw.v; return v>6?'#34D399':v>3?'#86EFAC':'#D1FAE5'; }, width:({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{const ca=chart?.chartArea;return ca?(ca.width/5)-6:20;}, height:({chart}:{ chart?: { chartArea?: { width: number; height: number } } })=>{ const ca=chart?.chartArea;return ca?(ca.height/4)-6:20;} }] };
export const pmsTreemap = { datasets:[{ tree:[ { value:30, label:'Engineering' }, { value:20, label:'Sales' }, { value:15, label:'HR' }, { value:10, label:'Finance' } ], key:'value', groups:['label'], backgroundColor:'#BFDBFE', spacing:0.5, labels:{ display:true, formatter:(ctx:{ raw: { g: string } })=>ctx.raw.g } }] };
