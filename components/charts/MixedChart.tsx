...
interface MixedChartProps { title: string; data: any; height?: number; }
...
export function MixedChart({ title, data, height = 300 }: MixedChartProps) {
  ...
  return (
    <div>
      ...
      <div style={{ height }}>
        <Chart type='bar' data={data} options={options} ref={(chart: any) => { canvasRef.current = chart?.canvas || null; }} />
      </div>
    </div>
  );
}
